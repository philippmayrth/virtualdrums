...
extension ImmersiveView {
    ...
    func handleDrumClick(entity: Entity, worldPosition: SIMD3<Float>) {
        guard simulatorStickState != nil else {
    ...
    func setupSimulatorStick(content: RealityViewContent) async {
        ...
        simulatorStickState = StickState(stickEntity: stickEntity, collidingEntity: stickEntity)
    }
    ...
    func startSimulatorSweep(at worldPosition: SIMD3<Float>) {
        ...
        simulatorSweepTask = Task { @MainActor in
            guard let simStick = simulatorStickState?.stickEntity else { return }

            let stickID = ObjectIdentifier(simStick)
            stickLastPosition[stickID] = simStick.position(relativeTo: nil)

            simulatorStickPosition = worldPosition
            simStick.setPosition(simulatorStickPosition, relativeTo: nil)

            await runSimulatorSweep(from: worldPosition, stick: simStick)
        }
    }
